...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const entries: MetadataRoute.Sitemap = [
    ...
    { url: `${baseUrl}/estado`, lastModified: new Date(), changeFrequency: "daily", priority: 0.6 },
    { url: `${baseUrl}/auditoria`, lastModified: new Date(), changeFrequency: "monthly", priority: 0.7 },
  ...
  const articulos = await sanityFetch<{ slug: string }[]>(
    `*[_type == "articulo" && activo == true && defined(slug.current)]{ "slug": slug.current }`
  )

  if (articulos) {
    for (const s of articulos) {
      entries.push({
