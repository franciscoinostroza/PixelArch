...
export function RegistrarPagoHitoButton({ hitoId, hitoTitulo, proyectoTitulo, precioUsd, precioArs, size = "sm" }: Props) {
  ...
  return (
    <>
      <Button size={size} variant="default" onClick={() => setOpen(true)}>
        Registrar pago
      </Button>
            ...
            {success ? (
              ...
              <>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <label htmlFor="rph-monto" className="block text-xs text-text-dim mb-1">
                  ...
                  <div>
                    <label htmlFor="rph-metodo" className="block text-xs text-text-dim mb-1">Método</label>
                    <select id="rph-metodo" value={metodo} onChange={(e) => setMetodo(e.target.value)} className="w-full rounded-lg border border-border bg-bg px-3 py-2.5 text-sm text-text font-mono">
                      {METODOS_PAGO.map((m) => <option key={m} value={m}>{METODO_LABEL[m]}</option>)}
                    </select>
                  </div>

                  <div>
                    <label htmlFor="rph-nota" className="block text-xs text-text-dim mb-1">Nota (opcional)</label>
                    <input id="rph-nota" value={nota} onChange={(e) => setNota(e.target.value)} className="w-full rounded-lg border border-border bg-bg px-3 py-2.5 text-sm text-text" placeholder="N° de operación, detalle..." />
                  </div>

                  <label className="flex items-center gap-2 text-sm text-text-dim">
                    <input type="checkbox" checked={marcarPagado} onChange={(e) => setMarcarPagado(e.target.checked)} style={{ accentColor: "#8b5cf6" }} />
                    Marcar el hito como pagado
                  </label>
                  <label className="flex items-center gap-2 text-sm text-text-dim">
