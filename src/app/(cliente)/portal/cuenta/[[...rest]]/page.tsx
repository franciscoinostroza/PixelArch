  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-white/60">Información de tu perfil y cuentas conectadas.</p>
      </div>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-white/70 truncate">{primaryEmail?.emailAddress}</p>
          </div>
        </div>
      </div>
            ...
            >
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                {email.verification.status === "verified" ? (
                  ...
                  <span className="text-[11px] text-[#f59e0b] font-medium">Unverified</span>
                )}
              </div>
            </div>
          ))}
            ...
            {accounts.map((acc, i) => (
              ...
              >
                <div className="flex items-center gap-2 min-w-0">
                  <span className="text-sm text-white truncate">{providers[acc.provider] ?? acc.provider}</span>
                  {acc.emailAddress && (
                    <span className="text-sm text-white/60 truncate">• {acc.emailAddress}</span>
                  )}
                </div>
                <span className="text-[11px] text-[#34d399] font-medium shrink-0">Connected</span>
              </div>
            ))}
          </div>
        </div>
