  ...
  return (
    <section className="proceso" id="proceso">
      ...
      <div className="wrap">
        ...
        <div className="timeline" id="timeline">
          <div className="timeline-track">
            <div className="timeline-track-fill" id="trackFill" />
            <div className="timeline-dot" id="trackDot" />
          </div>

          {displaySteps.map((step, i) => (
            <div className="step" data-step key={i}>
              <span className="step-ghost" aria-hidden="true">{step.num}</span>
              <div className="step-marker" dangerouslySetInnerHTML={{ __html: step.icon }} />
              <div className="step-content">
                <span className="step-num">{step.num} · {step.label}</span>
                <h3>{step.title}</h3>
                <p>{step.desc}</p>
              </div>
            </div>
      ...
      <style>{`
        ...
        .step-content p { color: var(--color-text-dim); font-size: 0.94rem; max-width: 50ch }
        ...
        @keyframes marker-pulse {
          ...
          100% { box-shadow: 0 0 0 6px rgba(34,211,238,0.14) }
        }
